...
3. **AARRR-stage-specific skills** — `onboarding`, `signup`, `emails`, `referrals`, `product-pricing`, etc. The "Marketing operations stack" (Section 11) maps these to AARRR stages.
...
4. **What the client is currently doing** — by channel, with status (working / not / not yet determined).
...
- **`user-research-synthesis`** — Deepens the ICP and voice-of-customer inputs that feed Section 2 (Strategic frame).
...
- **`product-pricing`** — Deep work on Section 8 (Revenue).
